...
export default function TerbaruPage() {
    ...
    const [currentPage, setCurrentPage, isHydrated] = usePaginationState("latest-page", 1);
    ...
    const fetchManga = useCallback(async (page: number) => {
        try {
            ...
            // Fetch with tag=update for latest updates
            const response = await fetch(`/api/manga?tag=update&page=${page}`);
